...
namespace truth_table
{
    public class Truth_table
    {
        public Truth_table(string reverse_polish_notation)
        {
            ...
            Stick_sknf();
            Make_sticked_sknf();
            Minimize_by_сounting_table_method(sdnf, sticked_sdnf);
            Minimize_by_сounting_table_method(sknf, sticked_sknf);
            Minimize_by_counting_method(sticked_sdnf);
            Minimize_by_counting_method(sticked_sknf);
            Minimize_by_Karnaugh_map();
        }


        private string input_str, sknf = "", sdnf = "", sticked_sdnf = "", sticked_sknf = "", min_sdnf_by_table = "", min_sknf_by_table = "", min_sdnf_by_counting = "",min_sknf_by_counting = "", min_sdnf_Karno = "", min_sknf_Karno = "";
        ...
        private void Print_min_sdnf_phase(List<string> lexemes_)
        {
            ...
            Console.WriteLine(sticked_sdnf);
        }
        private void Print_min_sknf_phase(List<string> lexemes_)
        {
            ...
            Console.WriteLine(sticked_sknf);
        }
        ...
        private void Stick_sdnf()
        {
            try
            {
                if (sdnf != "")
                {
                    ...
                    while (true)
                    {
                        Console.WriteLine("Фаза склеивания СДНФ");
            ...
            for (int j = 1; j < table_for_method[0].Count; j++)
            {
                for (int i = 1; i < table_for_method.Count; i++)
                {
                    ...
                    if (i == table_for_method.Count - 1 && counter_one == 1)
                        if(!lexemes.Contains(table_for_method[index][0]))
    ...
    private void Minimize_by_Karnaugh_map()
        ...
        public string Min_sdnf_Karno
        {
            get
            {
                return min_sdnf_Karno;
            }
        }
        public string Min_sknf_Karno
        {
            get
            {
                return min_sknf_Karno;
            }
        }

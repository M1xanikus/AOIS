...
namespace truth_table
{
    public class Truth_table
    {
        ...
        private int Calculate_results(List<int> list, string input_string)
        {
            ...
            {
                if ((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || ch == '0' || ch == '1' || ch == '2' || ch == '1') // added 0 1
                {
                    stack.Push(ch);
                    continue;
                }
        ...
        private List<string> Make_lexemes(string str_sdnf_or_sknf)
        {
            ...
            for (int i = 0; i < str_sdnf_or_sknf.Length; i++)
            {
                ...
                if ((str_sdnf_or_sknf[i] >= 'a' && str_sdnf_or_sknf[i] <= 'z') || (str_sdnf_or_sknf[i] >= 'A' && str_sdnf_or_sknf[i] <= 'Z') || (str_sdnf_or_sknf[i] >= '1' && str_sdnf_or_sknf[i] <= '9') && str_sdnf_or_sknf[i - 1] != '!')
                {
            ...
            for (int i = 0; i < tuple_list.Count; i++)
            {
                ...
                for (int j = 1; j < lex_length; j++)
                {
                    if (lexemes_[first][j] == ')' && lexemes_[second][j] == ')')
                    {
                        ...
                        if (!lexemes_.Contains(new_lex))
                        {
                            if (new_lex.Length < 6)
                            {
                                string buf = "";
                                while (buf.Length != 6-new_lex.Length)
                                    buf = buf.Insert(0, "X");
                                new_lex = new_lex.Insert(new_lex.Length-1,buf); // NEED TO FIX
                                if (lexemes_.Contains(new_lex))
                                    break;
                            }
                    ...
                    if (lexemes_[first][j] == 'X' && lexemes_[second][j] == 'X')
                ...
                // Ensure the new lexeme is 6 characters long
            ...
            for (int i = 0; i < lex_count - 1; i++)
            {
                ...
                for (int j = k; j < lex_count; j++)
                {
                    ...
                    for (int l = 1; l < length_lex; l++)
                    {
                        ...
                        {
                            make_new_lexeme = true; // make skleika
                            break;
                        }
